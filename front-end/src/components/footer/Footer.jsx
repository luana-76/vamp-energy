import './styleFooter.css';

import Logo from '../../assets/logo.png';
import { useLocation } from 'react-router-dom';

export function Footer(){

    const location = useLocation();

    // Verifica se estamos na rota de cadastro
    const isCadastroPage = location.pathname === '/cadastro'; 

    return(

        <footer className={isCadastroPage ? 'footerCadastro' : ''}>

            <div id='campoImagem'>

                <img src={Logo} alt='logo V'/>

            </div>

            <div id='campoInformacao'>

                <h3>Informações</h3>
                <a href=''>Saiba sobre o projeto</a>
                
            </div>

            <div id='contatosDev'>

                <h3>Contatos</h3>

                <ul>

                    <li>Telefone: <a target='_blank' href='tel:0 0000-0000'>0 0000-0000</a></li>
                    <li>Email: <a target='_blank' href='mailto:luana_maria@hotmail.com'>luana_maria@hotmail.com</a></li>

                </ul>
                
            </div>

            <div id='navegacaoExternaSocial'>

                <a href='https://www.instagram.com/printf.luana._/' target='_blank'>

                    <img src='data:image/png;base64,' alt='Instagram'/>

                </a>

                <a href='https://www.linkedin.com/in/luanamaria-dev/' target='_blank'>

                    <img src='data:image/png;base64,' alt='Linkedin'/>

                </a>

                <a href='https://github.com/luana-76' target='_blank'>

                    <img src='data:image/png;base64,' alt='Github'/>

                </a>

            </div>

        </footer>

    )
}